...
<div class="ns-portal">
	...
	<div class="cc-wrap">
		<div class="cc-cards">
			<div class="cc-card cc-card-total"><div class="cc-card-label">Total Orders</div><div class="cc-card-value"><?php echo esc_html( number_format_i18n( $cards['total'] ) ); ?></div></div>
			<div class="cc-card cc-card-pending"><div class="cc-card-label">Pending</div><div class="cc-card-value"><?php echo esc_html( number_format_i18n( $cards['pending'] ) ); ?></div></div>
			<div class="cc-card cc-card-booked"><div class="cc-card-label">Booked</div><div class="cc-card-value"><?php echo esc_html( number_format_i18n( $cards['booked'] ) ); ?></div></div>
			<div class="cc-card cc-card-transit"><div class="cc-card-label">In Transit</div><div class="cc-card-value"><?php echo esc_html( number_format_i18n( $cards['in-transit'] ) ); ?></div></div>
			<div class="cc-card cc-card-delivered"><div class="cc-card-label">Delivered</div><div class="cc-card-value"><?php echo esc_html( number_format_i18n( $cards['delivered'] ) ); ?></div></div>
		</div>
		...
		<form method="get" class="cc-filters" action="<?php echo esc_url( $self ); ?>">
			<input type="search" name="s" value="<?php echo esc_attr( $filters['search'] ); ?>" placeholder="Search order, customer…" />
			...
			<table class="cc-table">
				<thead>
					<tr>
						...
						<th>Order</th><th>Customer</th><th>City</th><th>Amount</th><th>Payment</th>
						<th>Shipment</th><th>AWB</th><th>Date</th><th>Actions</th>
					</tr>
				</thead>
				<tbody>
				<?php if ( empty( $result['orders'] ) ) : ?>
					<tr><td colspan="10" class="cc-empty">No orders found.</td></tr>
				<?php else : ?>
					<?php
					foreach ( $result['orders'] as $wc_order ) :
						...
						<tr>
							...
							<td><a href="<?php echo esc_url( $durl ); ?>"><strong>#<?php echo esc_html( $wc_order->get_order_number() ); ?></strong></a></td>
