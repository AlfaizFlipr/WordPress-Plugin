...
<div class="wrap cc-wrap">
	<div class="cc-topbar">
		...
		<div>
			...
			<?php if ( ! $awb ) : ?>
				<button class="cc-btn cc-btn-primary cc-push" data-order="<?php echo esc_attr( $order->id() ); ?>">Push to Delhivery</button>
			<?php else : ?>
				...
				<button class="cc-btn cc-btn-danger cc-cancel" data-order="<?php echo esc_attr( $order->id() ); ?>">Cancel Shipment</button>
			<?php endif; ?>
		</div>
	</div>
	...
	<div class="cc-grid-3">
		<div class="cc-panel">
			<h2>Customer</h2>
			<p><strong><?php echo esc_html( trim( $wc->get_billing_first_name() . ' ' . $wc->get_billing_last_name() ) ); ?></strong></p>
		...
		<div class="cc-panel">
			<h2>Shipment</h2>
			<p><strong>Status:</strong> <span class="cc-badge cc-badge-<?php echo esc_attr( $order->get_ship_status() ); ?>"><?php echo esc_html( $order->get_ship_status_label() ); ?></span></p>
			<p><strong>Courier:</strong> <?php echo esc_html( $wc->get_meta( '_cc_courier' ) ?: '—' ); ?></p>
			<p><strong>AWB:</strong> <?php echo $awb ? '<code>' . esc_html( $awb ) . '</code>' : '—'; ?></p>
			<p><strong>Payment:</strong> <?php echo $order->is_cod() ? 'COD (₹' . esc_html( $order->cod_amount() ) . ')' : 'Prepaid'; ?></p>
			<?php if ( $awb ) : ?>
				<p><a class="cc-btn cc-btn-sm" target="_blank" href="<?php echo esc_url( $order->get_tracking_url() ); ?>">Open tracking page</a></p>
			<?php endif; ?>
		</div>
	</div>
	...
	<div class="cc-grid-2">
		<div class="cc-panel">
			...
			<table class="cc-table cc-table-inner">
				...
				<?php endforeach; ?>
				<tr class="cc-row-total"><td colspan="2">Order Total</td><td><?php echo wp_kses_post( $wc->get_formatted_order_total() ); ?></td></tr>
				</tbody>
			</table>
		</div>
					...
					foreach ( $notes as $note ) :
						...
						<div class="cc-tl-item">
							...
							<div class="cc-tl-body">
								...
								<div class="cc-tl-time"><?php echo esc_html( $note->date_created ? $note->date_created->date_i18n( 'd M Y, H:i' ) : '' ); ?></div>
							</div>
						</div>
						<?php
					endforeach;
