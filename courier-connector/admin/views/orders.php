...
if (!function_exists('cc_courier_badge')) {
	function cc_courier_badge($name_or_key, $muted = false)
	{
		$label = isset(CC_Courier_Registry::all()[$name_or_key]) ? CC_Courier_Registry::all()[$name_or_key] : $name_or_key;
		$cls = $muted ? 'cc-courier-muted' : ('DTDC' === $label ? 'cc-courier-dtdc' : ('Delhivery' === $label ? 'cc-courier-delhivery' : 'cc-courier-muted'));
		return '<span class="cc-courier-badge ' . esc_attr($cls) . '">' . esc_html($label) . '</span>';
	}
}
...
<div class="wrap cc-wrap">
	<div class="cc-topbar">
		...
		<button class="cc-btn cc-btn-primary" id="cc-bulk-push" type="button">Push selected</button>
	</div>
		...
		<table class="cc-table">
			...
			<tbody>
				<?php if (empty($result['orders'])): ?>
					<tr>
						<td colspan="13" class="cc-empty">No orders found.</td>
					</tr>
				<?php else: ?>
					<?php
					foreach ($result['orders'] as $wc_order):
						...
						<tr>
							...
							<td><a
									href="<?php echo esc_url($detail_url); ?>"><strong>#<?php echo esc_html($wc_order->get_order_number()); ?></strong></a>
							</td>
							...
							<td class="cc-actions">
								<?php if (!$awb): ?>
									<span class="cc-push-group">
										<select class="cc-courier-select" data-order="<?php echo esc_attr($o->id()); ?>">
											<?php foreach (CC_Courier_Registry::all() as $ck => $cl): ?>
												<option value="<?php echo esc_attr($ck); ?>" <?php selected(CC_Shipment::resolve_courier($o), $ck); ?>><?php echo esc_html($cl); ?>
												</option>
											<?php endforeach; ?>
										</select>
