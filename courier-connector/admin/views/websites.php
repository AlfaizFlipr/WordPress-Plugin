...
<div class="wrap cc-wrap">
	...
	<div class="cc-grid-2">
		<div class="cc-panel">
			...
			<form method="post">
				...
				<p><label>Store name<br><input type="text" name="store_name" class="cc-input" placeholder="Fashion Store" /></label></p>
				<p><label>Store URL<br><input type="url" name="store_url" class="cc-input" placeholder="https://fashionstore.com" required /></label></p>
				<p><label>Callback base URL (for AWB push-back)<br><input type="url" name="callback_url" class="cc-input" placeholder="https://fashionstore.com/wp-json/courier/v1/" /></label></p>
				<p><label>Client login email (optional — creates a Client Portal account)<br><input type="email" name="client_email" class="cc-input" placeholder="owner@fashionstore.com" /></label></p>
	...
	<div class="cc-tablewrap">
		<table class="cc-table">
			<thead>
				<tr><th>Store</th><th>URL</th><th>API Key</th><th>Client Login</th><th>Status</th><th>Synced</th><th>Last Sync</th><th>Actions</th></tr>
			</thead>
			<tbody>
			<?php if ( empty( $stores ) ) : ?>
				<tr><td colspan="8" class="cc-empty">No stores connected yet.</td></tr>
			<?php else : ?>
				<?php foreach ( $stores as $s ) : ?>
					<tr>
						<td><strong><?php echo esc_html( $s->store_name ); ?></strong></td>
						<td><a href="<?php echo esc_url( $s->store_url ); ?>" target="_blank"><?php echo esc_html( $s->store_url ); ?></a></td>
						...
						<td>
							...
							<?php else : ?>
								<form method="post" style="display:flex;gap:4px;">
									...
									<input type="email" name="client_email" class="cc-input" style="padding:4px 6px;font-size:12px;" placeholder="client email" required />
									<button class="cc-btn cc-btn-sm" type="submit">Link</button>
								</form>
							<?php endif; ?>
						</td>
						<td><?php echo 'active' === $s->status ? '<span class="cc-badge cc-badge-delivered">Active</span>' : '<span class="cc-badge cc-badge-cancelled">Disabled</span>'; ?></td>
						...
						<td class="cc-actions">
							<form method="post" style="display:inline">
								...
								<?php endif; ?>
								<button class="cc-btn cc-btn-sm cc-btn-danger" name="sub_action" value="delete" onclick="return confirm('Delete this store?');">Delete</button>
							</form>
						</td>
					</tr>
				<?php endforeach; ?>
